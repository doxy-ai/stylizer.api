...
fn compute(@builtin(global_invocation_id) id: vec3<u32>) {
	...
	textureStore(nextMipLevel, id.xy, color);
		...
		auto_release newTexture = device.create_texture({.format = this->texture_format(), .usage = usage() | usage::CopyDestination | usage::Storage, .size = size, .mip_levels = mip_levels, .samples = samples()});
		newTexture.copy_from(device, *this, {}, {}, {}, 0, 1);

		auto previousView = newTexture.create_mip_view(0);
		auto nextView = newTexture.create_mip_view(1);
		std::array<texture, 2> textures {texture{}, texture{}};
		textures[0].view = previousView; textures[1].view = nextView;
		...
		newTexture.create_view();
		newTexture.sampler = std::exchange(sampler, nullptr);

		release();
		return (*this) = std::move(newTexture);
	}
}