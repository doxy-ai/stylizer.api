...
fn compute(@builtin(global_invocation_id) id: vec3<u32>) {
	...
	textureStore(nextMipLevel, id.xy, color);
		...
		auto_release newTexture = device.create_texture({.format = this->texture_format(), .usage = usage() | usage::CopyDestination, .size = size, .mip_levels = mip_levels, .samples = samples()});
