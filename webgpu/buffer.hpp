...
namespace stylizer::api::webgpu {
	struct buffer: public api::buffer { STYLIZER_API_GENERIC_AUTO_RELEASE_SUPPORT(buffer);
		...
		size_t size() override { return buffer_.getSize(); }
		...
		std::future<void*> map_async(api::device& device, bool for_writing = false, size_t offset = 0, std::optional<size_t> size = {}) override {
			static std::promise<void*> res; res = {};
			static constexpr auto set_promise_value = [](std::promise<void*>& res, wgpu::Buffer buffer, size_t offset, size_t size, bool for_writing) {
				if(for_writing) res.set_value((void*)buffer.getMappedRange(offset, size));
				else res.set_value((void*)buffer.getConstMappedRange(offset, size));
			};
		...
		void* map(api::device& device, bool for_writing = false, size_t offset = 0, std::optional<size_t> size = {}) override {
