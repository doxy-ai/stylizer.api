...
namespace stylizer::api::webgpu {
	...
	public:
		...
		api::command_encoder& bind_compute_pipeline(api::device& device, const api::compute_pipeline& pipeline_, bool release = false) override {
			...
			if(release) deferred_to_release.emplace_back([pipeline = std::move(pipeline)]() mutable {
				pipeline.release();
			});
			return *this;
		}

		api::command_encoder& bind_compute_group(api::device& device, const api::bind_group& group_, bool release = false, std::optional<size_t> index_override = {}) override {
