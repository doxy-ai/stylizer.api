...
export module stylizer.graphics.stub:compute_pipeline;
...
namespace stylizer::graphics::stub {
	...
	export struct compute_pipeline: public graphics::compute_pipeline { STYLIZER_GRAPHICS_GENERIC_AUTO_RELEASE_SUPPORT(compute_pipeline); STYLIZER_GRAPHICS_MOVE_TEMPORARY_TO_HEAP_DERIVED_METHOD(compute_pipeline);
		...
		static stub::compute_pipeline create(graphics::device& device, pipeline::entry_point entry_point, const std::string_view label = "Stylizer Compute Pipeline") { STYLIZER_THROW("Not implemented yet!"); }

		stub::bind_group create_bind_group(graphics::device& device, size_t index, std::span<const bind_group::binding> bindings, std::string_view label = "Stylizer Bind Group") { STYLIZER_THROW("Not implemented yet!"); }
