...
export module stylizer.graphics.stub:render_pipeline;
...
namespace stylizer::graphics::stub {
    ...
    export struct render_pipeline : public graphics::render_pipeline { STYLIZER_GRAPHICS_GENERIC_AUTO_RELEASE_SUPPORT(render_pipeline); STYLIZER_GRAPHICS_MOVE_TEMPORARY_TO_HEAP_DERIVED_METHOD(render_pipeline);
		...
		stub::bind_group create_bind_group(graphics::device& device, size_t index, std::span<const bind_group::binding> bindings, std::string_view label = "Stylizer Render Bind Group") { STYLIZER_THROW("Not implemented yet!"); }
