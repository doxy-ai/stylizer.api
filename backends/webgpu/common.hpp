...
namespace stylizer::api::webgpu {
	...
	inline WGPUTextureUsage to_webgpu_texture(usage usage) {
		...
		if(out == 0) STYLIZER_API_THROW(std::string("Failed to find texture usage: ") + magic_enum::enum_name(usage));
		return out;
	}
	...
	inline comparison_function from_webgpu(WGPUCompareFunction func) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find compare function: ") + magic_enum::enum_name(func));
		}
		std::unreachable();
	}
	...
	inline texture_format from_webgpu(WGPUTextureFormat format) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find texture format: ") + magic_enum::enum_name(format));
		}
	}
	...
	inline std::string format_to_string(WGPUTextureFormat format) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find texture format: ") + std::to_string(format));
		}
	}
} // namespace stylizer::api::webgpu
