...
namespace stylizer::graphics::webgpu {
	...
	export struct texture : public graphics::texture { STYLIZER_GRAPHICS_GENERIC_AUTO_RELEASE_SUPPORT(texture); STYLIZER_GRAPHICS_MOVE_TEMPORARY_TO_HEAP_DERIVED_METHOD(texture);
		...
		static webgpu::texture create(graphics::device& device, const create_config& config = {}) { STYLIZER_THROW("Not implemented yet!"); }
		static webgpu::texture create_and_write(graphics::device& device, std::span<const std::byte> data, const data_layout& layout, create_config config = {}) { STYLIZER_THROW("Not implemented yet!"); }

		webgpu::texture_view create_view(graphics::device& device, const view::create_config& config = {}) const { STYLIZER_THROW("Not implemented yet!"); }
