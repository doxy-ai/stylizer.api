...
namespace stylizer::api::webgpu {
	...
	inline surface::present_mode from_webgpu(WGPUPresentMode mode) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find present mode: ") + magic_enum::enum_name(mode));
		}
		std::unreachable();
	}
	...
	inline alpha_mode from_webgpu(WGPUCompositeAlphaMode mode) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find alpha mode: ") + magic_enum::enum_name(mode));
		}
		std::unreachable();
	}
