...
namespace stylizer::graphics::webgpu {
	...
	surface::config surface::determine_optimal_default_config(graphics::device& device_, vec2u size)  {
	...
	graphics::surface& surface::configure(graphics::device& device_, const config& config)  {
		auto& device = confirm_webgpu_type<webgpu::device>(device_);
	...
	texture surface::next_texture(graphics::device& device) {
	...
	graphics::texture& surface::next_texture(temporary_return_t, graphics::device& device)  {
		static webgpu::texture out;
		return out = next_texture(device);
	}

	texture_format surface::configured_texture_format(graphics::device&)  {
		return configured_format;
	}

	graphics::surface& surface::present(graphics::device& device)  {
