...
namespace stylizer::api::webgpu {
	...
	inline WGPUBufferUsage to_webgpu(usage usage) {
		...
		if(out == 0) STYLIZER_API_THROW(std::string("Failed to find buffer usage: ") + magic_enum::enum_name(usage));
		return out;
	}
	...
	std::byte* buffer::map(api::device& device, std::optional<bool> for_writing /* = false */, std::optional<size_t> offset /* = 0 */, std::optional<size_t> size /* = {} */) {
		...
		while(!future.valid()) confirm_webgpu_type<webgpu::device>(device).process_events();
