...
namespace stylizer::api::webgpu {
	...
	inline enum texture_view::create_config::aspect from_webgpu(WGPUTextureAspect aspect){
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find texture aspect: ") + std::string(magic_enum::enum_name(aspect)));
		}
		std::unreachable();
	}
	...
	inline texture::address_mode from_webgpu(WGPUAddressMode mode) {
		switch (mode) {
		case WGPUAddressMode_Repeat: return texture::address_mode::Repeat;
		case WGPUAddressMode_MirrorRepeat: return texture::address_mode::MirrorRepeat;
		case WGPUAddressMode_ClampToEdge: return texture::address_mode::ClampToEdge;
		default:
			STYLIZER_API_THROW(std::string("Failed to find address mode: ") + std::string(magic_enum::enum_name(mode)));
		}
		std::unreachable();
	}

	inline WGPUAddressMode to_webgpu(texture::address_mode mode) {
		switch (mode) {
		case texture::address_mode::Repeat: return WGPUAddressMode_Repeat;
		case texture::address_mode::MirrorRepeat: return WGPUAddressMode_MirrorRepeat;
		case texture::address_mode::ClampToEdge: return WGPUAddressMode_ClampToEdge;
		}
		std::unreachable();
	}
	...
	texture texture::create(api::device& device_, const create_config& config /* = {} */) {
	...
	texture_view texture::create_view(api::device& device, const view::create_config& config /* = {} */) const {
		return texture_view::create(device, *this, config);
	}
	api::texture_view& texture::create_view(temporary_return_t, api::device& device, const view::create_config& config /* = {} */) const {
		static webgpu::texture_view view;
		return view = create_view(device, config);
	}

	const api::texture_view& texture::full_view(api::device& device, bool treat_as_cubemap /* = false */) const {
		if (view) return view;
	...
	texture_format texture::texture_format() const {
	...
	api::texture& texture::configure_sampler(api::device& device_, const sampler_config& config /* = {} */) {
		auto& device = confirm_webgpu_type<webgpu::device>(device_);
	...
	api::texture& texture::copy_from(api::device& device_, const api::texture& source_, std::optional<vec3u> destination_origin /* = {{0, 0, 0}} */, std::optional<vec3u> source_origin /* = {{0, 0, 0}} */, std::optional<vec3u> extent_override /* = {} */, std::optional<size_t> min_mip_level /* = 0 */, std::optional<size_t> mip_levels_override /* = {} */) {
		auto& device = confirm_webgpu_type<webgpu::device>(device_);
