...
namespace stylizer::graphics::webgpu {
	...
	texture texture::create(graphics::device& device_, const create_config& config /* = {} */) {
	...
	texture texture::create_and_write(graphics::device& device, std::span<const std::byte> data, const data_layout& layout, create_config config /* = {} */) {
		using namespace magic_enum::bitwise_operators;
	...
	texture_view texture::create_view(graphics::device& device, const view::create_config& config /* = {} */) const {
		return texture_view::create(device, *this, config);
	}
	graphics::texture_view& texture::create_view(temporary_return_t, graphics::device& device, const view::create_config& config /* = {} */) const  {
		static webgpu::texture_view view;
		return view = create_view(device, config);
	}

	const graphics::texture_view& texture::full_view(graphics::device& device, bool treat_as_cubemap /* = false */) const  {
		if (view) return view;
	...
	texture::format texture::texture_format() const  {
		return from_webgpu(wgpuTextureGetFormat(texture_));
	}

	graphics::usage texture::usage() const  {
	...
	graphics::texture& texture::configure_sampler(graphics::device& device_, const sampler_config& config /* = {} */)  {
		auto& device = confirm_webgpu_type<webgpu::device>(device_);
	...
	bool texture::sampled() const  {
		return sampler;
	}

	graphics::texture& texture::write(graphics::device& device_, std::span<const std::byte> data, const data_layout& layout, vec3u extent, std::optional<vec3u> origin_ /* = {{ 0, 0, 0 }} */, size_t mip_level /* = 0 */)  {
	...
	graphics::texture& texture::copy_from(graphics::device& device_, const graphics::texture& source_, std::optional<vec3u> destination_origin /* = {{0, 0, 0}} */, std::optional<vec3u> source_origin /* = {{0, 0, 0}} */, std::optional<vec3u> extent_ /* = {} */, std::optional<size_t> min_mip_level /* = 0 */, std::optional<size_t> mip_levels_ /* = {} */)  {
		auto& device = confirm_webgpu_type<webgpu::device>(device_);
		...
		copy_texture_to_texture_impl(e, *this, source, destination_origin.value_or(vec3u{0, 0, 0}), source_origin.value_or(vec3u{0, 0, 0}), extent_.value_or(vec3u{0, 0, 0}), min_mip, mip_levels_.value_or(source.mip_levels() - min_mip));
		finish_and_submit(e, device.queue);
		return *this;
	}

	graphics::texture& texture::blit_from(graphics::device& device_, const graphics::texture& source_, std::optional<color32> clear_value /* = {} */, graphics::render_pipeline* pipeline_override /* = nullptr */, std::optional<size_t> vertex_count_override /* = {} */) {
		auto& device = confirm_webgpu_type<webgpu::device>(device_);
		auto& source = confirm_webgpu_type<webgpu::texture>(source_);
