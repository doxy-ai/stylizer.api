...
namespace stylizer::api::webgpu {
	...
	WGPUVertexFormat to_webgpu(enum render_pipeline::config::vertex_buffer_layout::attribute::format format) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find attribute format: ") + magic_enum::enum_name(format));
		}
	}
	...
	WGPUStencilOperation to_webgpu(depth_stencil_attachment::stencil_config::face_state::operation op) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find face operation: ") + magic_enum::enum_name(op));
		}
	}
	...
	WGPUBlendOperation to_webgpu(enum blend_state::operation op) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find blend operation: ") + magic_enum::enum_name(op));
		}
	}
	...
	WGPUBlendFactor to_webgpu(enum blend_state::factor op) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find blend factor: ") + magic_enum::enum_name(op));
		}
	}
	...
	WGPUPrimitiveTopology to_webgpu(enum render_pipeline::config::primitive_topology topology) {
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find primitive topology: ") + magic_enum::enum_name(topology));
		}
	}
	...
	WGPUCullMode to_webgpu(enum render_pipeline::config::cull_mode mode){
		...
		default:
			STYLIZER_API_THROW(std::string("Failed to find culling mode: ") + magic_enum::enum_name(mode));
		}
	}
	...
	render_pipeline render_pipeline::create(api::device& device_, const pipeline::entry_points& entry_points, std::span<const color_attachment> color_attachments /* = {} */, const std::optional<depth_stencil_attachment>& depth_attachment /* = {} */, const render_pipeline::config& config /* = {} */, const std::string_view label /* = "Stylizer Graphics Pipeline" */) {
		...
		if(depth_attachment) {
			...
			depth_state = WGPUDepthStencilState{
				.format = to_webgpu(depth_attachment->texture ? depth_attachment->texture->texture_format()
					: depth_attachment->view ? (depth_attachment->view->texture().texture_format()) : depth_attachment->texture_format),
				.depthWriteEnabled = to_webgpu(depth_attachment->should_store_depth && !depth_attachment->depth_readonly),
				.depthCompare = to_webgpu(depth_attachment->depth_comparison_function),
				.stencilFront = depth_attachment->stencil.has_value() ? WGPUStencilFaceState {
				...
				.stencilBack = depth_attachment->stencil.has_value() ? WGPUStencilFaceState {
				...
				.stencilReadMask = depth_attachment->stencil.has_value() ? depth_attachment->stencil->stencilReadMask : 0,
				.stencilWriteMask = depth_attachment->stencil.has_value() ? depth_attachment->stencil->stencilWriteMask : 0,
