...
namespace stylizer::graphics::webgpu {
    ...
    export struct buffer : public graphics::buffer { STYLIZER_GRAPHICS_GENERIC_AUTO_RELEASE_SUPPORT(buffer); STYLIZER_GRAPHICS_MOVE_TEMPORARY_TO_HEAP_DERIVED_METHOD(buffer);
		...
		static webgpu::buffer create(graphics::device& device, enum usage usage, size_t size, bool mapped_at_creation = false, const std::string_view label = "Stylizer Buffer") { STYLIZER_THROW("Not implemented yet!"); }

		static webgpu::buffer create_and_write(graphics::device& device, enum usage usage, std::span<const std::byte> data, size_t offset = 0, const std::string_view label = "Stylizer Buffer") { STYLIZER_THROW("Not implemented yet!"); }
		template<typename T>
		requires(!std::same_as<T, std::byte>)
		static webgpu::buffer create_and_write(graphics::device& device, enum usage usage, std::span<const T> data, size_t offset = 0, const std::string_view label = "Stylizer Buffer") {
			return create_and_write(device, usage, byte_span(data), offset, label);
		}

		static const webgpu::buffer& zero_buffer(graphics::device& device, enum usage usage = usage::Storage, size_t minimum_size = 0, graphics::buffer* just_released = nullptr) { STYLIZER_THROW("Not implemented yet!"); }
	...
	webgpu::buffer device::create_buffer(graphics::usage usage, size_t size, bool mapped_at_creation /* = false */, const std::string_view label /* = "Stylizer Buffer" */) { STYLIZER_THROW("Not implemented yet!"); }
	...
	webgpu::buffer device::create_and_write_buffer(graphics::usage usage, std::span<const std::byte> data, size_t offset /* = 0 */, const std::string_view label /* = "Stylizer Buffer" */) { STYLIZER_THROW("Not implemented yet!"); }
