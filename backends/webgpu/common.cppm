...
export module stylizer.graphics.webgpu.common;
...
namespace stylizer::graphics::webgpu {
	...
	export constexpr uint32_t magic_number = string2magic("webgpu");

	export WGPUInstance get_common_instance() {
	...
	export WGPUStringView to_webgpu(std::string_view view) { return {view.data(), view.size()}; }
	export std::string_view from_webgpu(WGPUStringView view) { return {view.data, view.length}; }

	export WGPUCommandEncoder create_command_encoder(WGPUDevice device, std::string_view label = "Temporary Encode") {
	...
	export void finish_and_submit(WGPUCommandEncoder e, WGPUQueue q) {
	...
	export bool wait_for_future(WGPUFuture future, std::chrono::nanoseconds timeout = std::chrono::milliseconds(1)) {
	...
	export bool wait_for_any_futures(std::span<WGPUFuture> futures, std::chrono::nanoseconds timeout = std::chrono::milliseconds(1)) {
	...
	export size_t wait_for_all_futures(std::span<WGPUFuture> futures, std::chrono::nanoseconds timeout = std::chrono::milliseconds(1)) {
		for (size_t i = 0; i < futures.size(); ++i)
	...
	export template<typename Tto, typename Tfrom>
	Tto& confirm_webgpu_type(Tfrom& ref) {
		auto& res = ref.template as<Tto>();
		assert(res.type == magic_number);
		return res;
	}
	export template<typename Tto, typename Tfrom>
	const Tto& confirm_webgpu_type(const Tfrom& ref) {
		auto& res = const_cast<Tfrom&>(ref).template as<Tto>();
		assert(res.type == magic_number);
		return res;
	}

	export WGPUColor to_webgpu(color32 c) { return WGPUColor{c.r, c.g, c.b, c.a}; }
	export color32 from_webgpu(WGPUColor c) { return color32{(stdmath::f32)c.r, (stdmath::f32)c.g, (stdmath::f32)c.b, (stdmath::f32)c.a}; }

	export WGPUOptionalBool to_webgpu(bool v) { return v ? WGPUOptionalBool_True : WGPUOptionalBool_False; }

	export usage from_webgpu_texture(WGPUTextureUsage usage) {
		...
		if(out == usage::Invalid) STYLIZER_THROW(std::string("Failed to find texture usage: ") + std::to_string(usage));
		return out;
	}

	export WGPUTextureUsage to_webgpu_texture(usage usage) {
		...
		if(out == 0) STYLIZER_THROW(std::string("Failed to find texture usage: ") + std::string(magic_enum::enum_name(usage)));
		return out;
	}

	export comparison_function from_webgpu(WGPUCompareFunction func) {
		switch (func) {
		...
		default:
			STYLIZER_THROW(std::string("Failed to find compare function: ") + std::string(magic_enum::enum_name(func)));
		}
		std::unreachable();
	}

	export WGPUCompareFunction to_webgpu(comparison_function func) {
		switch (func) {
	...
	export texture_format from_webgpu(WGPUTextureFormat format) {
		switch(format) {
		...
		default:
			STYLIZER_THROW(std::string("Failed to find texture format: ") + std::string(magic_enum::enum_name(format)));
		}
	}

	export WGPUTextureFormat to_webgpu(texture_format format) {
		switch(format) {
	...
	export std::string format_to_string(WGPUTextureFormat format) {
		switch (format) {
		...
		default:
			STYLIZER_THROW(std::string("Failed to find texture format: ") + std::string(std::to_string(format)));
		}
	}
} // namespace stylizer::graphics::webgpu
