...
namespace stylizer::api {
	...
	template<typename T>
	concept texture_concept = std::derived_from<T, texture> && requires(T t, device device, texture::create_config config) {
	...
	struct buffer {
		...
		virtual std::future<void*> map_async(device& device, bool for_writing = false, size_t offset = 0, std::optional<size_t> size = {}) = 0;
		virtual void* map(device& device, bool for_writing = false, size_t offset = 0, std::optional<size_t> size = {}) = 0;
	...
	struct command_encoder {
		...
		virtual command_encoder& bind_compute_pipeline(device& device, const compute_pipeline& pipeline, bool release = false) = 0;
		virtual command_encoder& bind_compute_group(device& device, const bind_group& group, bool release = false, std::optional<size_t> index_override = {}) = 0;
	...
	struct render_pass: public command_encoder {
		...
		virtual render_pass& bind_render_pipeline(device& device, const render_pipeline& pipeline, bool release = false) = 0;
		virtual render_pass& bind_render_group(device& device, const bind_group& group, bool release = false, std::optional<size_t> index_override = {}) = 0;
