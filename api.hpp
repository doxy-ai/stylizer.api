...
namespace stylizer::api {
	...
	struct depth_stencil_attachment {
		...
		std::optional<struct blend_state> blend_state = {};
	};
	...
	struct buffer {
		...
		virtual std::future<void*> map_async(api::device& device, bool for_writing = false, size_t offset = 0, std::optional<size_t> size = {}) = 0;
		virtual void* map(api::device& device, bool for_writing = false, size_t offset = 0, std::optional<size_t> size = {}) = 0;
	...
	struct render_pass: public command_encoder {
		...
		// virtual render_pass& bind_render_group(device& device, size_t index, const bind_group& group) = 0;
