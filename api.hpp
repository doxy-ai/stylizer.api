...
namespace stylizer {
	...
	signal<void(error_severity, std::string_view message, size_t error_tag)>& get_error_handler();
}
namespace stylizer::api {
	...
	struct command_buffer {
		signal<void()> deferred_to_release;
	...
	concept command_encoder_concept = std::derived_from<T, command_encoder> && requires(T t, device device, bool one_shot, const std::string_view label) {
		{ t.deferred_to_release } -> convertible_to_pointer<signal<void()>>;
	...
	struct render_pipeline : public pipeline {
		...
		struct config {
			...
			using attribute_format = enum vertex_buffer_layout::attribute::format;
