...
export module stylizer.graphics;

import std.compat;
import magic_enum;
import slcross;
import stdmath.vector;
import stdmath.color;

import stylizer.errors;
import stylizer.auto_release;
import stylizer.flags;
import stylizer.convertible_to_pointer;
import stylizer.spans;

namespace stylizer::graphics {

	export using bool32 = uint32_t;

	export using vec2u = stdmath::vector<size_t, 2>;

	export using vec3u = stdmath::vector<size_t, 3>;

	export using color32 = stdmath::color32;

	export using color8 = stdmath::color8;

	export using spirv = std::vector<uint32_t>;

	export using spirv_view = std::span<uint32_t>;

	#include "texture_format.partial"
	...
	export enum class shader_language {
		SPIRV,
		GLSL,
		Slang,
		WGSL,
	...
	export struct color_attachment {
	...
	export struct depth_stencil_attachment {
	...
	export struct temporary_return_t {};

	export constexpr temporary_return_t temporary_return = {};
	...
	export struct texture {
		using format = graphics::texture_format;
	...
	export struct shader {
		...
		inline static T create_from_source(device& device, language lang, stage stage, const std::string_view source, std::optional<const std::string_view> entry_point_ = "main", const std::string_view label = "Stylizer Shader") {
			...
			break; case language::Slang: {
				...
				module_name.erase(std::remove_if(module_name.begin(), module_name.end(), [](char c) { return std::isspace(c); }), module_name.end());
	...
	export template<typename Treturn>
	...
	export struct render_pass : public command_encoder_base<render_pass> {
		using blend_state = graphics::blend_state;
		using color_attachment = graphics::color_attachment;
		using depth_stencil_attachment = graphics::depth_stencil_attachment;
	...
	export struct render_pipeline : public pipeline {
		using blend_state = graphics::blend_state;
		using color_attachment = graphics::color_attachment;
		using depth_stencil_attachment = graphics::depth_stencil_attachment;
