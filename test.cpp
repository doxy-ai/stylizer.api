...
int main() {
	...
	if (!glfwInit()) {
		errors(stylizer::error::severity::Error, "Failed to initialize GLFW", 0);
		return -1;
	}
	glfwSetErrorCallback(+[](int type, const char* message) {
		stylizer::get_error_handler()(stylizer::error::severity::Error, std::string { "GLFW Error: " } + message, type);
	});
	...
	if (!window) {
		errors(stylizer::error::severity::Error, "Failed to create GLFW window", 0);
		return -1;
	}
